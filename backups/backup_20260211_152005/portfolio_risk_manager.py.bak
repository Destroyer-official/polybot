...
class PortfolioRiskManager:
    ...
    def check_can_trade(
        ...
        max_position = min(
            self.current_capital * self.max_position_size_pct,
            self._get_remaining_heat() * Decimal('0.5')  # Use max 50% of remaining heat
        )
        ...
        elif heat_pct + (proposed_size / self.current_capital) > self.max_portfolio_heat:
            can_trade = False
            reason = f"Portfolio heat too high: {heat_pct*100:.1f}% + {proposed_size/self.current_capital*100:.1f}% > {self.max_portfolio_heat*100}%"
        ...
        # 5. Market exposure check
        elif self._get_market_exposure(market_id) + proposed_size > \
             self.current_capital * self.max_position_per_market_pct:
            can_trade = False
            reason = f"Market exposure limit for {market_id}"
        
        # 6. Position size check
        elif proposed_size > max_position:
            can_trade = False
            reason = f"Position too large: ${proposed_size} > max ${max_position}"
        
        # 7. Minimum capital check
        elif self.current_capital < Decimal('1.0'):
            can_trade = False
